...
import { Check, CheckCircle2, ChevronRight, Copy, Expand, Printer, Share2, Undo2 } from "lucide-react";
...
export default function PaymentCard({ session, index, compact = false, customerMode = false, onTogglePaid, onNext, onFullscreen }: Props) {
  ...
  return <article className={`print-slip animate-enter rounded-[14px] border bg-white shadow-card transition dark:bg-[#171a20] ${payment.paid ? "border-emerald-300 dark:border-emerald-500/40" : "border-slate-200 dark:border-white/10"} ${compact ? "p-4" : "p-5 sm:p-6"}`}>
    ...
    <button type="button" onClick={() => onFullscreen(index)} className="mx-auto mt-5 block cursor-zoom-in" aria-label="Open QR fullscreen"><QRCodeDisplay value={link} size={customerMode ? 260 : compact ? 150 : 208} /></button>
    <div className="mt-3 text-center"><p className="text-xs font-medium text-slate-500">Scan with any supported UPI app</p><p className="mt-3 font-semibold">{session.merchant.name}</p>{!customerMode && <><p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">{session.merchant.upiId}</p>{session.note && <p className="mt-1 text-xs text-slate-400">{session.note}</p>}</>}</div>
